...
@Component({
  selector: 'app-valider-etudiant',
  templateUrl: './valider-etudiant.component.html',
  styleUrls: ['./valider-etudiant.component.css']
})
export class ValiderEtudiantComponent implements OnInit{
...
enregister(id:number,app:number,app2:number,app3:number,no1:string,no2:string,no3:string,liste:Liste){
  ...
  this.httpService.postDt(va).subscribe(d =>{
    ...
    alert("Vous avez accepté ce groupe");
...
refuse(id:number){
  ...
  alert("Vous avez refusé ce groupe")
  this.router.navigate(['espace-demande', this.id])
